...
class Tree:
    ...
    def serialize(self, for_treant=True) -> str:
        def encapsulate(d):
            data = {}
            name, value, image, children, *_ = d.values()
            if not (name):
                name = ""
            if not (value):
                value = ""
    ...
    def search_for_path(self, path_from_root, allow_unnamed=False):
        ...
        """
        assert (path_from_root[0] == 0.0 or path_from_root[0] == 1.0), TreeException(
            "{} is an invalid root value.".format(path_from_root[0]))  # noqa
